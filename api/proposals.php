...
if ($method === 'GET') {
    ...
    foreach ($rows as &$r) {
        $r['lines'] = json_decode($r['line_items'] ?? '[]', true);
        $r['date']  = $r['proposal_date'];
        $r['vat']   = (bool)$r['vat'];
    }
    jsonOut($rows);
}
...
if ($method === 'POST') {
    ...
    $stmt = $db->prepare('INSERT INTO proposals (number,contact,company,status,proposal_date,expiry,vat,notes,line_items) VALUES (?,?,?,?,?,?,?,?,?)');
...
if ($method === 'PUT' && $id) {
    $b = body();
    $db->prepare('UPDATE proposals SET contact=?,company=?,status=?,expiry=?,vat=?,notes=?,line_items=? WHERE id=?')
